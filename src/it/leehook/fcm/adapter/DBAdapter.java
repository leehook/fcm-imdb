...
public class DBAdapter {
    ...
    private static final String CREATE_SERIEA = "create table if not exists serieA (codice text primary key, nome text not null);";
    private static final String DROP_SERIEA = "delete from serieA";

    private static final String CREATE_COPPA = "create table if not exists coppa (codice text primary key, nome text not null, giornata text);";
    private static final String DROP_COPPA = "delete from coppa";
    ...
    /**
     * Cancella le righe dalla tabella specificata in base alla query fornita in input
     * 
     * @param tableName
     * @param whereClause
     * @return
    ...
    /**
     * Esegue una query sulla tabella specificata in base ai criteri della whereClause
     * 
     * @param tableName
     * @param whereClause
     * @return
	...
	String nome = "";
	Cursor curs = db.query(tableName, new String[] { Constants.NOME }, Constants.CODICE + "='" + codice + "'", null, null, null, null);
	...
	Integer ruolo = 0;
	Cursor curs = db.query(Constants.TABLE_PLAYER, new String[] { Constants.RUOLO }, Constants.CODICE + "='" + codice + "'", null,
		null, null, null);
	while (curs.moveToNext()) {
	    ruolo = curs.getInt(0);
	}
     ...
     */
    public Integer isMatchPlayed(String giornata, String sqCasa, String sqTr, String codiceCompetizione) {
	Cursor curs = db.query(Constants.TABLE_MATCH, new String[] { Constants.GIOCATA }, Constants.NUMERO + "='" + giornata + "' AND "
		+ Constants.SQ_CASA + "='" + sqCasa + "' AND " + Constants.SQ_TRASF + "='" + sqTr + "' AND " + Constants.ID_CAMPIONATO
     ...
     * @return
     */
    public boolean updateData(String tableName, Map<String, String> newColumnValues, String whereClause) {
    ...
    /**
     * Verifica se la tabella della coppa sia già popolata
     */
    public boolean isCoppaFull() {
	Cursor c = getData(Constants.TABLE_CUP, null, null);
	if (c.getCount() > 0) {
	    c.close();
	    return true;
	} else {
    ...
    /**
     * Verifica se la tabella della coppa sia già popolata per un certo turno
     */
    public String isCoppaFull(String code) {
	Cursor c = getData(Constants.TABLE_CUP, Constants.CODICE + "='" + code + "' AND " + Constants.GIORNATA + " IS NOT NULL", null);
	String turno = "";
	if (c.moveToNext()) {
	    turno = c.getString(1);
	}
    ...
    /**
     * Verifica se i tabellini di una specifica giornata sono stati gia' salvati o meno
     * 
     * @param numGiornata
     * @return
     */
    public boolean isTabSaved(Integer numGiornata) {
	Cursor c = getData(Constants.TABLE_TAB, Constants.NUMERO_GIORNATA + "=" + numGiornata, null);
	if (c.getCount() > 0) {
	    c.close();
	    return true;
	} else {
	...
	Integer giornata = 1;
	Cursor c = getData(Constants.TABLE_PREFS, Constants.NOME + "='" + Constants.PROSSIMA_GIORNATA + "'", null);
	while (c.moveToNext()) {
	    giornata = c.getInt(1);
	}
	c.close();
	...
	Integer idSq = 1;
	Cursor c = getData(Constants.TABLE_SQ,
		Constants.NOME + " LIKE '%" + (nomeSquadra.length() > 4 ? StringUtils.capitalize(nomeSquadra) : nomeSquadra.toUpperCase())
			+ "%'", null);
	while (c.moveToNext()) {
	    idSq = c.getInt(0);
	}
	...
	Integer idLega = 1;
	Cursor c = db.query(Constants.TABLE_SQ, new String[] { Constants.ID_CAMPIONATO }, null, null, null, null, null);
	if (c.moveToNext()) {
	    idLega = c.getInt(0);
	}
	c.close();
	...
	Cursor c = getData(Constants.TABLE_TAB, Constants.NUMERO_GIORNATA + "=" + giornata + " AND " + Constants.ID_SQUADRA + "="
		+ idSquadra, null);
	if (c.getCount() == 25) {
	    c.close();
	    return true;
	...
	Cursor c = db.query(Constants.TABLE_SQ, new String[] { Constants.NOME }, null, null, null, null, Constants.NOME);
	while (c.moveToNext()) {
	...
	if (!StringUtils.isBlank(nomeSquadra)) {
	    Cursor curs = getData(Constants.TABLE_MATCH, Constants.NUMERO + "=" + giornata + " AND " + Constants.ID_CAMPIONATO + "="
		    + getIdCampionato() + " AND (" + Constants.SQ_CASA + "='" + nomeSquadra + "' OR " + Constants.SQ_TRASF + "='"
		    + nomeSquadra + "')", null);
	    String result = "";
	    while (curs.moveToNext()) {
		Integer golCasa = Integer.valueOf(curs.getString(2));
		Integer golTr = Integer.valueOf(curs.getString(5));
		if (StringUtils.equalsIgnoreCase(nomeSquadra, curs.getString(1))) {
		    if (golCasa > golTr) {
			result = "Complimenti! " + nomeSquadra + " ha vinto " + golCasa + " a " + golTr + " nella " + giornata
				+ "a giornata contro " + curs.getString(4) + "!";
		    } else if (golCasa < golTr) {
			result = "Peccato! " + nomeSquadra + " ha perso " + golTr + " a " + golCasa + " nella " + giornata
				+ "a giornata contro " + curs.getString(4);
		    } else {
			result = nomeSquadra + " ha pareggiato " + golCasa + " a " + golTr + " nella " + giornata + "a giornata contro "
				+ curs.getString(4);
		    }
		} else if (StringUtils.equalsIgnoreCase(nomeSquadra, curs.getString(4))) {
		    if (golCasa < golTr) {
			result = "Complimenti! " + nomeSquadra + " ha vinto " + golTr + " a " + golCasa + " nella " + giornata
				+ "a giornata contro " + curs.getString(1) + "!";
		    } else if (golCasa > golTr) {
			result = "Peccato! " + nomeSquadra + " ha perso " + golCasa + " a " + golTr + " nella " + giornata
				+ "a giornata contro " + curs.getString(1);
		    } else {
			result = nomeSquadra + " ha pareggiato " + golCasa + " a " + golTr + " nella " + giornata + "a giornata contro "
				+ curs.getString(1);
