...
public class DbUtils {
	...
	public static void populateClassifica(Context ctx) throws Exception {
		...
		dbAdapter.open();
		URL url = null;
		String classificaUrl = host + File.separator + Constants.JS_FOLDER
				+ File.separator + Constants.CLASSIFICA_FILE;
			...
			while ((strLine = in.readLine()) != null) {
				if (strLine.contains("]=new C(")) {
					...
					if (Integer.valueOf(singolaRiga[2]) == idCampionato) {
						Integer idSquadra = Integer.valueOf(singolaRiga[0]
								.substring(singolaRiga[0].length() - 1));
						...
						values.put(
								...
								""
										+ Integer.valueOf(Integer
												.valueOf(singolaRiga[7])
												+ Integer
														.valueOf(singolaRiga[10])));
						values.put(
								Constants.PAREGGIATE,
								""
										+ Integer.valueOf(Integer
												.valueOf(singolaRiga[8])
												+ Integer
														.valueOf(singolaRiga[11])));
						values.put(
								Constants.PERSE,
								""
										+ Integer.valueOf(Integer
												.valueOf(singolaRiga[9])
												+ Integer
														.valueOf(singolaRiga[12])));
						values.put(
								Constants.GOL_FATTI,
								""
										+ Integer.valueOf(Integer
												.valueOf(singolaRiga[13])
												+ Integer
														.valueOf(singolaRiga[15])));
						values.put(
								Constants.GOL_SUBITI,
								""
										+ Integer.valueOf(Integer
												.valueOf(singolaRiga[14])
												+ Integer
														.valueOf(singolaRiga[16])));
						dbAdapter.updateData(Constants.TABLE_SQ, values,
	...
	public static void populateGiornate(Context ctx) throws Exception {
		dbAdapter = new DBAdapter(ctx);
		dbAdapter.open();
		String risultatiUrl = host + File.separator + Constants.JS_FOLDER
				+ File.separator + Constants.CALENDARIO_FILE;
		Integer idCampionato = dbAdapter.getIdCampionato();
		URL url = null;
		try {
			...
			Boolean mapFull = Utils.getMappaSquadre().size() == numeroSquadre ? true
					: false;
			...
			while ((strLine = in.readLine()) != null) {
				if (!mapFull && strLine.startsWith("var")) {
					if (startLine != null) {
						...
						for (int i = startLine.intValue(); i < startLine
								+ numeroSquadre.intValue(); i++) {
							String codiceSq = strLine.substring(
									strLine.indexOf('x'), strLine.indexOf('='));
							String nomeSq = strLine.substring(
									strLine.indexOf('"') + 1,
									strLine.length() - 1);
					...
					Boolean giornataGiocata = Integer.valueOf(singolaRiga[3]) == 1 ? true
							: false;
					Integer giocataDb = dbAdapter.isMatchPlayed(
							singolaRiga[10],
							Utils.getMappaSquadre().get(singolaRiga[15]), Utils
									.getMappaSquadre().get(singolaRiga[16]),
							singolaRiga[6]);
					values.put(Constants.NUMERO, singolaRiga[10]);
					values.put(Constants.GIOCATA, singolaRiga[3]);
					if (!prossimaSettata && singolaRiga[3].equals("0")
							&& Integer.valueOf(singolaRiga[6]) == idCampionato) {
						Map<String, String> pref = new HashMap<String, String>();
						pref.put(Constants.VALORE, singolaRiga[10]);
						dbAdapter.updateData(Constants.TABLE_PREFS, pref,
								Constants.NOME + "='"
										+ Constants.PROSSIMA_GIORNATA + "'");
					...
					case 0:
						if (giornataGiocata) {
							...
							values.put(Constants.SQ_CASA, Utils
									.getMappaSquadre().get(singolaRiga[15]));
							values.put(Constants.GOL_CASA, singolaRiga[17]);
							values.put(Constants.PNT_CASA, singolaRiga[21]);
							values.put(Constants.SQ_TRASF, Utils
									.getMappaSquadre().get(singolaRiga[16]));
							values.put(Constants.GOL_TRASF, singolaRiga[18]);
							values.put(Constants.PNT_TRASF, singolaRiga[22]);
							values.put(Constants.ID_SQUADRA_CASA,
									singolaRiga[11]);
							values.put(Constants.ID_SQUADRA_TRASF,
									singolaRiga[12]);
							values.put(
									Constants.MOD_CASA,
									new BigDecimal(singolaRiga[21])
											.subtract(
													new BigDecimal(
															singolaRiga[19]))
							...
							values.put(
									Constants.MOD_TRASF,
									new BigDecimal(singolaRiga[22])
											.subtract(
													new BigDecimal(
															singolaRiga[20]))
											.toBigInteger().toString());
							dbAdapter.updateData(Constants.TABLE_MATCH, values,
									Constants.NUMERO + "=" + singolaRiga[10]
											+ " AND "
											+ Constants.ID_SQUADRA_CASA + "="
											+ singolaRiga[11] + " AND "
											+ Constants.ID_SQUADRA_TRASF + "="
											+ singolaRiga[12] + " AND "
											+ Constants.ID_CAMPIONATO + "="
											+ singolaRiga[6]);
						}
						break;
					case 1:
					...
					case 2:
						values.put(Constants.ID_CAMPIONATO, singolaRiga[6]);
						values.put(Constants.SQ_CASA, Utils.getMappaSquadre()
								.get(singolaRiga[15]));
						values.put(Constants.SQ_TRASF, Utils.getMappaSquadre()
								.get(singolaRiga[16]));
	...
	public static void populateProssimeFormazioni(Context ctx) throws Exception {
		...
		dbAdapter.open();
		URL url = null;
		String formPrefix = host + File.separator + Constants.JS_FOLDER
				+ File.separator + Constants.FORMAZIONI_FILE;
		Map<String, String> values = new HashMap<String, String>();
		Integer prossimaGiornata = dbAdapter.getProssimaGiornata();
		Integer idCampionato = dbAdapter.getIdCampionato();
		dbAdapter.deleteData(Constants.TABLE_TAB, Constants.NUMERO_GIORNATA
				+ "=" + prossimaGiornata);
		try {
			...
			for (String strLine : sb.split("\n")) {
				if (strLine.contains("]=new Z(")) {
					values.clear();
					String[] singolaRiga = strLine.split(",");
					if (!dbAdapter.isTeamComplete(prossimaGiornata,
							Integer.valueOf(singolaRiga[1]))) {
						values.put(Constants.NUMERO_GIORNATA, ""
								+ prossimaGiornata);
	...
	public static void populateSerieA(Context ctx) throws Exception {
		...
		dbAdapter.deleteData(Constants.TABLE_PREFS, Constants.NOME + "='"
				+ Constants.PROSSIMA_GIORNATA + "'");
		...
		host = getSprefs(ctx).getString(Constants.PREF_HOST,
		...
		try {
			...
			if (!dbAdapter.isSerieAFull()) {
				String datiUrl = host + File.separator + Constants.JS_FOLDER
						+ File.separator + Constants.SERIE_A_FILE;
				URL url = null;
				url = new URL(datiUrl);
				InputStream inputStream = (InputStream) url.getContent();
				BufferedReader in = new BufferedReader(new InputStreamReader(
	...
	public static void populateSquadre(Context ctx) throws Exception {
		...
		try {
			...
			if (!dbAdapter.isSquadreFull()) {
				URL url = null;
				String roseUrl = host + File.separator + Constants.JS_FOLDER
						+ File.separator + Constants.SQUADRE_FILE;
				url = new URL(roseUrl);
				InputStream inputStream = (InputStream) url.getContent();
				BufferedReader in = new BufferedReader(new InputStreamReader(
						inputStream));
				...
				for (String strLine : sb.split("\n")) {
					if (strLine.contains("]=new F(")) {
						String[] singolaRiga = strLine.split(",");
						Map<String, String> values = new HashMap<String, String>();
						values.put(Constants.ID_SQUADRA, singolaRiga[0]
								.substring(singolaRiga[0].length() - 1));
						values.put(Constants.NOME,
								singolaRiga[1].replace('"', ' '));
						values.put(Constants.PRESIDENTE,
								singolaRiga[2].replace('"', ' '));
						values.put(Constants.MAIL,
								singolaRiga[6].replace('"', ' '));
						values.put(
								Constants.CREDITI,
								singolaRiga[8].substring(0,
										singolaRiga[8].length() - 1));
						...
						if (Integer.valueOf(singolaRiga[5]) == 0) {
							...
							dbAdapter.updateData(Constants.TABLE_PLAYER,
									values, Constants.CODICE + "='"
											+ singolaRiga[2] + "'");
						}
					}
				}
				// Inserisco il codice delle competizioni
				String competizioniUrl = host + File.separator
						+ Constants.JS_FOLDER + File.separator
						+ Constants.COMPETIZIONI_FILE;
				url = new URL(competizioniUrl);
				inputStream = (InputStream) url.getContent();
				in = new BufferedReader(new InputStreamReader(inputStream));
				sb = Utils.getStringFromStringBuffer(in);
				for (String strLine : sb.split("\n")) {
					if (StringUtils.contains(strLine,"Campionato")) {
						String rigaCampionato = strLine.split(",")[0];
						Integer idCampionato = Integer
								.valueOf(rigaCampionato
										.substring(rigaCampionato
												.lastIndexOf("(") + 1));
						Map<String, String> values = new HashMap<String, String>();
						values.put(Constants.ID_CAMPIONATO, "" + idCampionato);
	...
	public static void populateTabellini(Context ctx) throws Exception {
		...
		dbAdapter.open();
		URL url = null;
		String tabPrefix = host + File.separator + Constants.JS_FOLDER
				+ File.separator + Constants.RISULTATI_FILE;
		Map<String, String> values = new HashMap<String, String>();
		...
		try {
			for (int i = Integer.valueOf(getSprefs(ctx).getString(
					Constants.PREF_PRIMA, "1")); i < prossimaGiornata; i++) {
				...
				if (!dbAdapter.isTabSaved(i)) {
					...
					for (String strLine : sb.split("\n")) {
						if (strLine.contains("]=new T(")) {
							...
							if (!squadreInserite.contains(singolaRiga[1])) {
								values.put(Constants.ID_SQUADRA, singolaRiga[1]);
								String[] players = singolaRiga[3].substring(1,
										singolaRiga[3].length() - 1).split("%");
								String[] voti = singolaRiga[8].substring(1,
										singolaRiga[8].length() - 1).split("%");
								for (int j = 0; j < players.length; j++) {
									Integer tit = 0;
									values.put(Constants.CODICE,
											Constants.PL_PREFIX + players[j]);
									values.put(Constants.VOTO, voti[j]);
									if (j < 11) {
										tit = 1;
									}
									values.put(Constants.TITOLARE, "" + tit);
									dbAdapter.insertData(Constants.TABLE_TAB,
											values);
								}
					...
					for (String strLine : sb.split("\n")) {
						if (strLine.contains("]=new T(")) {
							String[] singolaRiga = strLine.split(",");
							dbAdapter.deleteData(Constants.TABLE_TAB,
									Constants.NUMERO_GIORNATA + "=" + i
											+ " AND " + Constants.ID_SQUADRA
											+ "=" + singolaRiga[1]);
							String[] players = singolaRiga[3].substring(1,
									singolaRiga[3].length() - 1).split("%");
							String[] voti = singolaRiga[8].substring(1,
	 ...
	 * @param tipoFoto
	 */
	private static void downloadImages(Context ctx, String tipoFoto)
			throws Exception {
		URL url = null;
		...
		if (size > 0) {
			...
			for (String s : squadre) {
				String nomeFile = s.toLowerCase().replaceAll("\\s+", "")
						+ (StringUtils.equals(tipoFoto, Constants.ALL_FOLDER) ? ".png"
								: ".gif");
				URI uri = new URI(
						"http",
						host.replace(Constants.HTTP, ""),
						File.separator
								...
								+ (StringUtils.equals(tipoFoto,
										Constants.ALL_FOLDER) ? ".png" : ".gif"),
						null);
				url = uri.toURL();
				InputStream inputStream = (InputStream) url.getContent();
				FileOutputStream fos = new FileOutputStream(Constants.IMG_PATH
						+ tipoFoto + File.separator + nomeFile);
	...
	public static void getVariabiliScadenza(Context ctx) {
		String host = getSprefs(ctx).getString(Constants.PREF_HOST,
				Constants.DEFAULT_HOST);
		if (!host.startsWith(Constants.HTTP)) {
			host = Constants.HTTP.concat(host);
		}
		...
		int year = 0;
		try {
			URL url = new URL(host + File.separator + Constants.JS_FOLDER
					+ File.separator + Constants.VARIABILI_FILE);
			InputStream inputStream = (InputStream) url.getContent();
			BufferedReader in = new BufferedReader(new InputStreamReader(
					inputStream));
			String strLine;
	...
	public static Date getLastMatchDate(Context ctx, Integer giornata) {
		...
		try {
			URL url = new URL(host + File.separator + Constants.JS_FOLDER
					+ File.separator + Constants.DATA_A_FILE);
			InputStream inputStream = (InputStream) url.getContent();
			BufferedReader in = new BufferedReader(new InputStreamReader(
					inputStream));
			String strLine;
			...
			if (!StringUtils.isBlank(dataStr)) {
				data = new SimpleDateFormat("MMMM' 'dd' 'yyyy' 'HH:mm",
						Locale.ENGLISH).parse(dataStr);
			}
		} catch (Exception e) {
			Toast.makeText(ctx, e.getMessage(), Toast.LENGTH_SHORT).show();
			Log.e(ctx.getClass().getSimpleName(), e.getMessage(), e);
