...
import it.leehook.fcm.impl.CalendarSwipeDetector;
...
public class Calendario extends ListActivity {
	...
	CalendarSwipeDetector calendarSwipeDetector = new CalendarSwipeDetector(this);
	...
	ArrayAdapter<Integer> spinnerArrayAdapter = new ArrayAdapter<Integer>(this, android.R.layout.simple_spinner_item, getGiornate());
	spinnerArrayAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
	spinner.setAdapter(spinnerArrayAdapter);
	/*
	 * Setto come valore di default dello spinner di ricerca giornata l'ultima giocata, a meno che oggi non sia il giorno della partita.
	 * In questo caso setto il valore della prossima giornata
	 */
	prossimaGiornata = adapter.getProssimaGiornata();
	int defaultValue = prossimaGiornata - Integer.valueOf(sPrefs.getString(Constants.PREF_PRIMA, "1")) - 1;
	Date lastMatchDate = DbUtils.getLastMatchDate(Calendario.this, (prossimaGiornata - 1));
	...
	cal2.set(Calendar.DAY_OF_MONTH,
		cal2.get(Calendar.DAY_OF_MONTH)
			+ (cal2.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY || cal2.get(Calendar.DAY_OF_WEEK) == Calendar.TUESDAY ? 2
				: 3));
	...
	spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
		...
		Utils.setGiornata(Integer.valueOf(parent.getItemAtPosition(pos).toString()));
		partite = getPartite();
	...
	RelativeLayout layout = (RelativeLayout) findViewById(R.id.layoutRisultati);
	layout.setOnTouchListener(calendarSwipeDetector);
    }
    ...
    public void aumentaGiornataSelezionata() {
	spinner.setSelection(spinner.getSelectedItemPosition() + 1);
    }

    public void diminuisciGiornataSelezionata() {
	spinner.setSelection(spinner.getSelectedItemPosition() - 1);
    }
	...
	Cursor curs = adapter.getData(Constants.TABLE_MATCH, Constants.NUMERO + "=" + Utils.getGiornata() + " AND "
	...
	Cursor curs = adapter.getData(Constants.TABLE_TAB, Constants.NUMERO_GIORNATA + "=" + Utils.getGiornata() + " AND "
		+ Constants.ID_SQUADRA + "=" + idSquadra, Constants.TITOLARE);
	startManagingCursor(curs);
	while (curs.moveToNext()) {
	    // Caso tabellino
	    if (giocata) {
	...
	for (int i = Integer.valueOf(sPrefs.getString(Constants.PREF_PRIMA, "1")); i <= Integer.valueOf(sPrefs.getString(
		Constants.PREF_ULTIMA, "38")); i++) {
	    list.add(i);
	}
	return list;
    }
