...
public class FCMActivity extends TabActivity {
	...
	private final static int TABELLINI = 3;
	private final static int CLASSIFICA = 4;
	private final static int GIORNATE = 5;
	private final static int FOTO = 6;
	private final static int FINISHING = 7;
	...
	/**
	 * Verifica il termine per l'invio della formazione e, se mancano meno di
	 * due ore, mostra un messaggio di warning
	 */
	private void checkResultsAndInvioFormazione() {
		if (!Utils.checkConn(getApplicationContext())) {
			...
			Boolean notifiche = sPrefs.getBoolean(Constants.PREF_NOTIFICHE,
					true);
			if (notifiche) {
				...
				if (scadenza != null) {
					...
					final String oraScadenza = cal.get(Calendar.HOUR_OF_DAY)
							+ ":"
							+ (cal.get(Calendar.MINUTE) == 0 ? "00" : cal
									.get(Calendar.MINUTE));
					...
					if (now.before(scadenza) && now.after(notificationDate)) {
						if (!adapter.isTeamComplete(prossimaGiornata,
								adapter.getIdSquadraByName(nomeSquadra))) {
							Toast.makeText(
									this,
									"Attenzione! Alle ore "
											+ oraScadenza
											+ " scade il termine per l'invio della formazione!",
									Toast.LENGTH_LONG).show();
						}
					} else {
						Date lastMatchDate = DbUtils.getLastMatchDate(
								FCMActivity.this, (prossimaGiornata - 1));
						...
						if (DateUtils.isSameDay(check, now)
								&& now.getHours() > 13) {
							Toast.makeText(
									FCMActivity.this,
									adapter.getLastResult(
											(prossimaGiornata - 1),
											StringUtils.trim(nomeSquadra)),
									Toast.LENGTH_LONG).show();
						}
					}
				}
	...
	public static void setTabColor(TabHost tabhost) {
		for (int i = 0; i < tabhost.getTabWidget().getChildCount(); i++) {
			tabhost.getTabWidget().getChildAt(i)
					.setBackgroundColor(Color.parseColor("#000000")); // unselected
		}
		tabhost.getTabWidget().getChildAt(tabhost.getCurrentTab())
				.setBackgroundColor(Color.parseColor("#FE2E2E")); // selected
	}
	...
	final Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			if (total >= 100) {
				dismissDialog(0);
				Toast.makeText(FCMActivity.this,
						getString(R.string.updateSuccessful),
						Toast.LENGTH_SHORT).show();
			}
		}
	};
		...
		public void run() {
			try {
				...
				if (PreferenceManager.getDefaultSharedPreferences(
						getBaseContext()).getBoolean(Constants.PREF_CLEAR_DB,
						false)) {
					...
					public void run() {
						...
						Toast.makeText(
								...
								getString(R.string.error)
										+ getString(R.string.checkhost) + error,
								Toast.LENGTH_LONG).show();
					}
				});
			}
	...
	private void resetUpdatePreferences() {
		...
		Map<String, Boolean> categories = (Map<String, Boolean>) sPrefs
				.getAll();
